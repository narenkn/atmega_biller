...
findFiles (uint8_t flag, uint8_t *fileName)
{
  ...
  while(1) {
	...
	if(dir->name[0] == EMPTY) { //indicates end of the file list of the directory
	  if(flag == DELETE)
	    transmitString_F((uint8_t *)PSTR("File does not exist!"));
	  return 0;   
	}
	if((dir->name[0] != DELETED) && (dir->attrib != ATTR_LONG_NAME)) {
	  if((flag == GET_FILE) || (flag == DELETE)) {
		...
		TX_NEWLINE;
		transmitString_F((uint8_t *)PSTR("Deleting.."));
		TX_NEWLINE;
		TX_NEWLINE;
		...
		while(1) {
		  ...
		  if(nextCluster > 0x0ffffff6) 
		    {transmitString_F((uint8_t *)PSTR("File deleted!"));return 0;}
	    ...
	    if((dir->attrib != 0x10) && (dir->attrib != 0x08)) {
	      transmitString_F ((uint8_t *)PSTR("FILE" ));
	      transmitString_F ((uint8_t *)PSTR("   "));
	      displayMemory (LOW, dir->fileSize);
	    }
	    else
	      transmitString_F ((uint8_t *)((dir->attrib == 0x10)? PSTR("DIR") : PSTR("ROOT")));
	  }
	}
      }
    }
    ...
    if(cluster > 0x0ffffff6)
      return 0;
    if(cluster == 0) {
      transmitString_F((uint8_t *)PSTR("Error in getting cluster"));
      return 0;
    }
  }
  return 0;
...
//***************************************************************************
uint8_t
readFile (uint8_t flag, uint8_t *fileName)
{
  struct dir_Structure *dir;
  ...
  while(1)
      ...
      for(j=0; j<sectorPerCluster; j++)
	{
      ...
      cluster = getSetNextCluster (cluster, GET, 0);
      if(cluster == 0) {transmitString_F((uint8_t *)PSTR("Error in getting cluster")); return 0;}
    }
  return 0;
}
...
//***************************************************************************
uint8_t
convertFileName (uint8_t *fileName)
{
  ...
  if(j>8) {transmitString_F((uint8_t *)PSTR("Invalid fileName..")); return 1;}
...
//************************************************************************************
uint8_t
writeFile (uint8_t *fileName)
{
  uint8_t j,k, data, error, fileCreatedFlag = 0, start = 0, appendFile = 0, sector=0;
  ...
  if(j == 1) 
	...
	{
	  TX_NEWLINE;
	  transmitString_F((uint8_t *)PSTR(" No free cluster!"));
	  return 1;
	}
  ...
  while(1) {
	...
	transmitString_F((uint8_t *)PSTR("End of Cluster Chain")); 
	return 1;
      }
    }
    if(cluster == 0) {
