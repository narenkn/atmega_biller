...
#define NVF_SALE_END_ADDR /* could be last-avail-addr+1 */		\
  (NVF_SALE_START_ADDR+(SALE_DATA_SIZEOF_NORM*(NVF_SALE_MAX_BILLS-1)))

#define NVF_PREV_SALE_RECORD(curAddr)			\
  ( (curAddr==NVF_SALE_START_ADDR) ?				\
    NVF_SALE_END_ADDR : (curAddr-SALE_DATA_SIZEOF_NORM) )
#define NVF_NEXT_SALE_RECORD(curAddr)			\
  ( ((curAddr+SALE_DATA_SIZEOF_NORM)<=NVF_SALE_END_ADDR) ?	\
    (curAddr+SALE_DATA_SIZEOF_NORM) : NVF_SALE_START_ADDR )
