...
SPIFlash::SPIFlash
(uint8_t slaveSelectPin, uint16_t jedecID)
{
...
SPIFlash::initialize()
{
  ...
  if (_jedecID == 0 || readDeviceId() == _jedecID) {
    ...
    spiTransfer(0);                     // Global Unprotect
    unselect();
    return true;
  }
  return false;
...
#else
  select();
  spiTransfer(SPIFLASH_IDREAD);
#endif
  uint16_t jedecid = spiTransfer(0) << 8;
  jedecid |= spiTransfer(0);
  unselect();
  return jedecid;
}
...
uint8_t
SPIFlash::readByte(uint32_t addr)
{
...
void
SPIFlash::readBytes(uint32_t addr, void* buf, uint16_t len)
{
  command(SPIFLASH_ARRAYREAD);
  spiTransfer(addr >> 16);
  spiTransfer(addr >> 8);
  spiTransfer(addr);
  spiTransfer(0); //"dont care"
  for (uint16_t i = 0; i < len; ++i)
    ((uint8_t*) buf)[i] = spiTransfer(0);
  unselect();
}

/// Send a command to the flash chip, pass TRUE for isWrite when its a write command
void
SPIFlash::command(uint8_t cmd, boolean isWrite)
{
#if defined(__AVR_ATmega32U4__) // Arduino Leonardo, MoteinoLeo
...
SPIFlash::busy()
{
  /*
  select();
  spiTransfer(SPIFLASH_STATUSREAD);
...
void
SPIFlash::writeByte(uint32_t addr, uint8_t byt)
{
...
///
void
SPIFlash::writeBytes(uint32_t addr, const void* buf, uint16_t len)
{
  uint16_t n;
  ...
  while (len>0)
  {
    ...
    for (uint16_t i = 0; i < n; i++)
      spiTransfer(((uint8_t*) buf)[offset + i]);
    unselect();
...
void
SPIFlash::blockErase4K(uint32_t addr)
{
...
void
SPIFlash::blockErase32K(uint32_t addr)
{
...
void
SPIFlash::blockErase64K(uint32_t addr)
{
