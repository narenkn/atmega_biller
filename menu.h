...
#define MENU_ITEMS \
  ...
  MENU_HIER(MENU_HIER_REPORTS) MENU_MODE(MENU_MSUPER|MENU_MNORMAL)  MENU_NAME("Show Bills  ") COL_JOIN MENU_FUNC(menuShowBill) COL_JOIN \
    ARG1(MENU_PR_FROM_DATE,  MENU_ITEM_DATE|MENU_ITEM_OPTIONAL) COL_JOIN ARG2(MENU_PR_ID, MENU_ITEM_ID|MENU_ITEM_OPTIONAL) /* FIXME: Need to provide facility to enter bill id */ ROW_JOIN \
...
uint8_t menuIndexItem(struct item *it);
...
#define DIAG_CPU_EE           (1<<4)
#define DIAG_STORE_I2C        (1<<5)
#define DIAG_TIMER            (1<<6)
#define DIAG_CPU_FL           (1<<7)
